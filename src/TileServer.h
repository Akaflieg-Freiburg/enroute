 ...
 * used for hillshading. Each set contains two MBTiles files, one for Africa and
 * one for Europe.
 */
...
public:
     ...
     *
     * This constructor sets up a new tile server. The server will serve the
     * static content found in the Qt ressource system under ":/static", as well
     * as sets of tiles added using the method addMbtilesFileSet().
     *
     * @param The URL under which the tile will be available.  If this URL is
     * empty, then the TileJSON generated by the server will direct clients to
     ...
     * through an reverse ssl-proxy under the Url "https://myserver.com" then
     * you should pass that URL here.
     *
...
public slots:
     ...
     * files must exist until the file set is removed or the sever is
     * destructed, or else replies to tile requests will yield undefined
     * results. The tile files are expected to agree in their metadata, and the
     * metadata (attribution, description, format, name, minzoom, maxzoom) is
     * read only from one of the files (a random one, in fact). If a tile is
     * contained in more than one of the files, the data is expected to be
     * identical in each of the files.
     *
