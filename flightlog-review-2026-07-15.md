...
### [ ] [D-002] updateFlight() silently discards trackFile, orphaning the IGC file
...
### [ ] [D-003] removeFlight() never deletes the flight's IGC file — permanent orphans
...
### [ ] [D-004] saveTrack() ignores I/O errors; caller destroys the in-memory track regardless
...
### [ ] [D-005] Full waypoint scan + O(n log n) distance-sort on every position update in flight
...
### [ ] [D-006] In-progress flight tracked by fragile list index — corruptible via normal UI use
...
### [ ] [D-007] showTrack() mutates displayed-track state before its failure check
...
### [ ] [D-008] load() silently discards a corrupt log, which the next save() then overwrites
...
### [ ] [D-009] Export failure indistinguishable from success — UI shares a 0-byte file with a success toast
...
### [ ] [D-010] Stale live track retained in RAM and on the map when settings toggle mid-flight
...
### [ ] [D-011] Unbounded track growth when the detector is stuck InFlight
...
### [ ] [D-012] trackRecording / showCurrentFlightTrace proxies break the NOTIFY contract in both directions
...
### [ ] [D-013] displayedTrackIndex value changes on list mutations without its NOTIFY firing
...
### [ ] [D-014] Flight-leg lifecycle split across three classes, held together by signal-ordering assumptions and a fabricated Idle state
...
### [ ] [D-016] Dead constant airfieldProximityM; the real thresholds are magic numbers in other places
...
### [ ] [D-017] endFlight() documented contract contradicts the implementation, in two headers
...
### [ ] [D-018] Seven-line detector state-reset block triplicated, plus a diverging partial variant
...
### [ ] [D-019] Duplicated export index-collection and duration-formatting logic
...
### [ ] [D-020] flights property copies the full list per read; delete-all is O(n²) with n disk writes
...
### [ ] [D-021] Live track path rebuilt from scratch on every recorded point, read twice by QML
...
### [ ] [D-022] Missing const / [[nodiscard]] on non-mutating export methods
...
### [ ] [D-023] Inconsistent PositionInfo passing: detector takes by value, recorder by const-ref
...
### [ ] [D-024] Abstract FlightDetector has one implementation and no swap API despite docs promising one
...
### [ ] [D-025] Dangling comment for a removed member
...
### [ ] [I-002] landingCount increments on LandingPhase entry — go-arounds without touchdown count as landings
...
### [ ] [I-003] IGC filename has minute resolution — same-minute saves silently overwrite
...
### [ ] [I-004] Detector signals carry both a QDateTime and its pre-formatted "HH:mm" string
...
### [ ] [I-005] Flight::operator== ignores coordinates and trackFile
...
### [ ] [I-006] FlightRecorder::clearTrack() changes trackGeoPath without emitting trackGeoPathChanged
...
### [ ] [I-007] Case-sensitive callsign matching in lastArrivalICAO and CSV aircraft dedup
...
### [ ] [I-009] trackFromIGC(): unchecked toInt() and altitude 0 conflated with "unknown"
...
### [ ] [I-010] Go-around path emits signals mid-transition — latent reentrancy hazard
