...
/*! \brief Satellite Navigator
  ...
  in formats suitable for GUI applications.  If no new SatNav fixes come in for one
  minute, the QGeoPositionInfo will cleared, and the status is set to Timeout
...
public:
    ...
    This property holds the raw altitude found in the last satnav fix, in feet above
    sea level.  Negative altitudes are possible.  In case no valid SatNav fix
    exists, or in case where no altitude information is reported, the property
  ...
  */
    ...
    This property holds the raw altitude found in the last SatNav fix, as a string of
    the form "2.943 ft".  In case no valid satnav fix exists, or in case where no
    altitude information is reported, the property is set to the value "-".
  ...
  */
    ...
    This property holds the altitude, defined as rawAltitude()+altitudeCorrection, in feet above
    sea level.  Negative altitudes are possible.  In case no valid SatNav fix
    exists, or in case where no altitude information is reported, the property
    is set to the value "0".
  */
   ...
   *
   * and stores altitudeCorrection in QSettings, so it can be used when the program is run next. If the rawAltitude cannot be read (e.g. when there is not satellite reception), this method does nothing.
   */
    void setAltitudeInFeet(int altitudeInFeet);

    /*! \brief Altitude reading from the last SatNav fix

    This property holds the altitude, defined as rawAltitude()+altitudeCorrection, as a string of
    the form "2.943 ft".  In case no valid SatNav fix exists, or in case where no
    altitude information is reported, the property is set to the value "-".
    ...
    */
    ...
    This property is set to "true" if the last SatNav fix contains altitude information.
    ...
    */
    ...
    This property holds the ground speed found in the last SatNav fix, in meters per second.
    In case no valid SatNav fix exists, the property is set to the value "-1.0",
    otherwise the number returned is never negative
    */
    ...
    */
    ...
    This property holds the last valid track found in the last SatNav fix.  This property differs from track, in that it remains unchanged when no track information can be found or when an error occurs. At
    the first start, this property is set to 0.  The property always satisfies 0 <= track < 360. The value is stored in a QSetting at destruction, and restored in the
    construction.
  ...
  */
    ...
    /*! \brief Returns the last SatNav fix received

    If no SatNav has been received for more than one minute, this method return an
    empty QGeoPositionInfo.
  */
